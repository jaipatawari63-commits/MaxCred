...
/* MaxCred demo UX layer: searchable wallet selector, six simple spend categories, approved upward-facing flying-horse mark. */
(function(){
  const cards = window.MAXCRED_CARDS;
  function enhance(){
    const cardBox=document.getElementById('cards');
    if(cardBox){
      cardBox.innerHTML=`<div class="wallet-picker">
        <div class="wallet-search-row"><input id="cardSearch" class="wallet-search" type="search" placeholder="Search your cards…" autocomplete="off"><button id="clearCards" class="clear-cards" type="button">Clear</button></div>
        <div id="cardResults" class="card-results"></div>
        <div class="selected-label">Your cards <span>Tap to remove</span></div>
        <div id="selectedCards" class="selected-cards"><span class="empty-wallet">No cards selected yet</span></div>
      </div>`;
      const results=document.getElementById('cardResults'), selected=document.getElementById('selectedCards'), search=document.getElementById('cardSearch');
      const selectedIds=new Set();
      function hiddenChoices(){
        cardBox.querySelectorAll('.hidden-choice').forEach(x=>x.remove());
        selectedIds.forEach(id=>{const c=cards.find(x=>x.id===id);if(!c)return;const b=document.createElement('button');b.type='button';b.className='choice selected hidden-choice';b.innerHTML=`<strong>${c.name}</strong>`;b.style.display='none';cardBox.appendChild(b);});
      }
      function render(){
        const q=(search.value||'').trim().toLowerCase();
        const matches=cards.filter(c=>!q||(`${c.name} ${c.issuer}`.toLowerCase().includes(q)));
        results.innerHTML=matches.map(c=>`<button type="button" class="card-result ${selectedIds.has(c.id)?'picked':''}" data-id="${c.id}"><span><b>${c.name}</b><small>${c.issuer} · ${c.network}</small></span><i>${selectedIds.has(c.id)?'✓':'+'}</i></button>`).join('') || '<div class="no-results">No matching card found.</div>';
        selected.innerHTML=selectedIds.size?[...selectedIds].map(id=>{const c=cards.find(x=>x.id===id);return `<button type="button" class="card-chip" data-remove="${id}">${c.name}<span>×</span></button>`}).join(''):'<span class="empty-wallet">No cards selected yet</span>';
        hiddenChoices();
        results.querySelectorAll('[data-id]').forEach(b=>b.onclick=()=>{const id=b.dataset.id;selectedIds.has(id)?selectedIds.delete(id):selectedIds.add(id);render();});
        selected.querySelectorAll('[data-remove]').forEach(b=>b.onclick=()=>{selectedIds.delete(b.dataset.remove);render();});
      }
      document.getElementById('clearCards').onclick=()=>{selectedIds.clear();render();search.focus();};
    ...
    if(spendings){
      const fields=[['Online & Shopping','online'],['Dining & Food','dining'],['Travel','travel'],['Fuel','fuel'],['UPI','upi'],['Bills & Other','utilities']];
      spendings.innerHTML=fields.map(([label,key])=>`<div class="spend"><label>${label}</label><input class="spend-input" data-key="${key}" type="number" min="0" inputmode="numeric" placeholder="₹0" value="0"></div>`).join('');
      spendings.querySelectorAll('input').forEach(i=>i.addEventListener('input',()=>{const total=[...spendings.querySelectorAll('input')].reduce((a,x)=>a+Math.max(0,Number(x.value||0)),0);document.getElementById('total').textContent='₹'+Math.round(total).toLocaleString('en-IN');}));
    }
